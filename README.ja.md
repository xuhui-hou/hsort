...
```bash
hsort --license YOUR_KEY      # 有効化
hsort --check-license         # 状態確認
```
...
HSORTは、システムの言語設定を自動検出して、ヘルプメッセージやエラーメッセージを日本語または英語で表示します。

### 自動検出

以下の順序で言語を検出します：

1. **環境変数 `HSORT_LANG`**（明示的な指定）
2. **Windows**: システムのUI言語設定
3. **Unix/Linux**: `locale.getdefaultlocale()` の結果
4. **環境変数**: `LANG`, `LANGUAGE`, `LC_ALL`, `LC_MESSAGES`

### 言語の明示的な指定

環境変数 `HSORT_LANG` を設定することで、言語を明示的に指定できます：
...
# 現在のセッションでのみ有効（新しいPowerShellウィンドウでは無効）
# 永続的に設定する場合：
...
**注意**: PowerShell では `set` コマンドは使用できません。必ず `$env:HSORT_LANG="en"` の形式を使用してください。

### デバッグモード

言語検出の問題を診断するには、環境変数 `HSORT_DEBUG_LANG` を設定してデバッグ情報を表示できます：
...
デバッグモードでは、環境変数の値や検出プロセスに関する情報が標準エラー出力に表示されます。
...
- **例外:**
  ...
  - `RuntimeError`: 無償版の入力ファイル合計サイズ上限を超えた場合など（CLI と同じ条件）
...
HSORT は無償利用枠と有償ライセンスを提供します。

### 🟢 無料版
- 入力ファイル合計サイズ：**100MB** まで
- 機能制限はありません（サイズ制限のみ）
...
ライセンス未アクティベート（キー未登録）の場合、**入力として指定した通常ファイルの合計サイズ**は **100 MiB（100 × 1024 × 1024 バイト）** を超えると、**CLI・Python API いずれも**実行できません。

- 上限はソースの `hsort/api.py` 内の定数 **`FREE_VERSION_MAX_TOTAL_INPUT_BYTES`** で定義されています。
- **`-W` で指定する最大メモリ（例: `-W100M`）とは別**です。`-W` はソートに使うメモリの上限であり、無償版の「入力ファイル合計サイズ」制限ではありません。
- 有効なライセンスをアクティベートすると、この合計サイズ制限は解除されます（利用許諾の内容に従います）。

## ライセンス

Copyright (c) 2015–2026 株式会社GPO

本プロジェクトは**オープンソースではありません**。ソフトウェアは**独占ライセンス（プロプライエタリ）**です。全文はリポジトリの **`LICENSE`** を参照してください。
